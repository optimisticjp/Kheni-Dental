...
const RATIO = {
  ...
  tall: "h-72 sm:h-auto sm:aspect-[4/3]",
} as const;

export function BranchMap({ location, className, size = "standard", shape = "rounded" }: { location: Location; className?: string; size?: keyof typeof RATIO; shape?: "rounded" | "smile" }) {
  return (
    <AerialFrame location={location} className={cn("group/map relative isolate overflow-hidden bg-cream-2", shape === "smile" ? "smile-lg" : "rounded-[1.75rem]", RATIO[size], className)}>
...
export function ExactLocationPanel({ location, className }: { location: Location; className?: string }) {
  ...
  return (
    <div className={cn(`mood-${location.hue} relative isolate overflow-hidden bg-m-tint p-5 text-ink sm:p-6`, className)}>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="font-display text-xl font-extrabold tracking-[-.03em]">{location.displayArea}</p>
        ...
        {verified && (
          <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-white px-2.5 py-1">
            ...
            <span className="text-sm font-bold">{location.google.rating}</span>
          </span>
        )}
      </div>
      <dl className="mt-4 space-y-2">
        <div className="flex gap-2.5">
          ...
          <MapPin className="mt-0.5 size-4 shrink-0 text-m-text" aria-hidden="true" />
          <dd className="t-small text-ink-soft">{location.address}</dd>
        </div>
        <div className="flex gap-2.5">
          <dt className="sr-only">Opening hours</dt>
          <Clock3 className="mt-0.5 size-4 shrink-0 text-m-text" aria-hidden="true" />
...
export function DirectionsButton({ location, placement, className }: { location: Location; placement: string; className?: string }) {
  return (
    <a href={directionsUrl(location)} target="_blank" rel="noreferrer" data-track="directions_click" data-placement={placement} data-branch={location.slug} className={cn("btn btn-ink", className)}>
      <MapPin className="size-4 text-butter" aria-hidden="true" />
      Get Directions
