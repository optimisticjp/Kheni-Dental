...
export function ConsultationForm({ international = false }: { international?: boolean }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    pushTrackingEvent({ event: "appointment_submit", placement: international ? "international_form" : "contact_form" });
    const message = [
      "Hello Kheni Elite, I would like to request a consultation.",
  ...
  return (
    <form onSubmit={submit} className="grid gap-4 rounded-[2rem] border border-gold/20 bg-card p-6 shadow-sm sm:p-8" id="book">
      ...
      <div>
        <label htmlFor="preference" className="text-sm font-medium">Preferred contact method</label>
      ...
      <p className="text-xs leading-5 text-muted-foreground">Please do not enter diagnosis, symptoms, medical history or other sensitive health information here. Clinical details can be discussed privately with the clinic.</p>
