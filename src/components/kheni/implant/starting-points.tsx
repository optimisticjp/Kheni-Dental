...
export function StartingPoints() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)] lg:gap-6">
        ...
        {startingPoints.map((item, index) => {
          ...
          return (
            <button
              ...
              className={cn(
                "ease-kheni flex min-h-16 items-start gap-3 rounded-2xl border p-3 text-left transition-colors duration-200 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-gold sm:p-4",
                selected ? "border-gold bg-gold-tint" : "border-line bg-white hover:border-line-strong",
              )}
            >
              <span aria-hidden="true" className={cn("mt-1 grid size-5 shrink-0 place-items-center rounded-full border-2", selected ? "border-gold bg-gold" : "border-line-strong")}>
      ...
      <div role="tabpanel" id="sp-panel" aria-labelledby={`sp-tab-${point.id}`} tabIndex={0} className="min-w-0 rounded-[1.5rem] border border-line bg-white p-5 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-gold sm:p-7">
        <h3 className="t-h3">{point.label}</h3>
        <p className="t-body mt-3 max-w-2xl text-ink-soft">{point.summary}</p>
        <div className="mt-6 grid gap-6 sm:grid-cols-2">
          <div>
            <p className="t-eyebrow text-gold-text">Questions worth asking</p>
          ...
          <div>
            <p className="t-eyebrow text-gold-text">Options usually discussed</p>
            <ul className="mt-3 space-y-2">
              {point.options.map((o) => (
