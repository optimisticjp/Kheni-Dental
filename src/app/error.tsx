...
export default function Error({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <section className="hue-coral field relative isolate flex min-h-[70vh] items-center overflow-hidden" style={{ ["--f1" as string]: "var(--coral-tint)", ["--f2" as string]: "var(--gold-tint)", ["--f3" as string]: "var(--sky-tint)" }}>
      <Container width="7xl" className="relative py-14">
        ...
        <div className="mt-6 flex flex-col gap-2.5 sm:flex-row sm:flex-wrap">
          <button type="button" onClick={reset} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-gold px-5 text-[.9375rem] font-semibold text-ink">
          ...
          <a href={`tel:${site.primaryPhoneHref}`} data-track="phone_click" data-placement="error_page" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white px-5 text-[.9375rem] font-semibold ring-1 ring-line">
            <Phone className="size-4 text-gold-text" aria-hidden="true" />
        ...
        <Link href="/" className="mt-6 inline-flex min-h-11 items-center text-sm font-semibold text-gold-text">
