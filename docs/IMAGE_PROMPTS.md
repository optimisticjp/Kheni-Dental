...
`public/brand/og-card.jpg` · landscape 1200×630

This is the picture that appears when the site is shared on WhatsApp, which
is where most of it will be shared.

**Currently the logo, not a photograph.** The card is generated by
`scripts/make-brand-assets.mjs`: the dark lockup on the site's ink under the gold
bloom. The brief below still stands for the day the clinic's own photography
arrives, but it should be a real photograph of a real Kheni reception by then, not
a generated one.
